...
class DNAErrorLab {
  ...
  setSequence(seq) {
    ...
    this.originalSequence = clean.length > 0 ? clean : 'CAGACAGC';
  ...
  introducePointMutation(targetIdx = null) {
    if (!this.originalSequence || this.originalSequence.length === 0) return;

    const bases = ['A', 'C', 'G', 'T'];
    const idx = targetIdx !== null ? targetIdx : Math.floor(Math.random() * this.originalSequence.length);
    ...
    const arr = this.corruptedSequence.split('');
    arr[idx] = newBase;
    this.corruptedSequence = arr.join('');
  ...
  cycleBaseAtIndex(idx) {
    if (idx < 0 || idx >= this.originalSequence.length) return;
    const bases = ['A', 'C', 'G', 'T'];
  ...
  introduceNoise(percentage) {
    ...
    const bases = ['A', 'C', 'G', 'T'];
    const count = Math.max(1, Math.round((this.originalSequence.length * percentage) / 100));

    // Choose unique random indices
    const indices = [];
    while (indices.length < count && indices.length < this.originalSequence.length) {
      const r = Math.floor(Math.random() * this.originalSequence.length);
      if (!indices.includes(r)) indices.push(r);
    }

    const arr = this.originalSequence.split('');
    indices.forEach(idx => {
      const origBase = arr[idx];
      const candidates = bases.filter(b => b !== origBase);
      const newBase = candidates[Math.floor(Math.random() * candidates.length)];
      arr[idx] = newBase;
      this.mutatedIndices.set(idx, { from: origBase, to: newBase });
   ...
   */
  decodeComparison() {
    const origDec = window.dnaCodec.decodeDNA(this.originalSequence);
    const corrDec = window.dnaCodec.decodeDNA(this.corruptedSequence);
